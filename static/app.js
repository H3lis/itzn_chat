...
function addBot(resp) {
  ...
  if (summaryMatch && detailsMatch) {
    ...
    // 요약 문단 생성 (항상 표시)
    const summaryDiv = el("div", "ans-summary");
    renderAnswer(summaryDiv, summaryText);
    ansBox.appendChild(summaryDiv);

    // 상세 문단 생성 (기본 접힘, max-height: 0)
    detailsDiv = el("div", "ans-details collapsible");
    renderAnswer(detailsDiv, detailsText);
    ansBox.appendChild(detailsDiv);

    // 더보기 버튼 생성
    btnMore = el("button", "btn-more", "자세한 설명 더보기 ▾");
    btnMore.addEventListener("click", (ev) => {
      ev.stopPropagation();
      if (detailsDiv.classList.contains("expanded")) {
        detailsDiv.classList.remove("expanded");
        btnMore.textContent = "자세한 설명 더보기 ▾";
        wrap.scrollIntoView({ behavior: "smooth", block: "nearest" });
      } else {
        detailsDiv.classList.add("expanded");
        btnMore.textContent = "간략히 보기 ▴";
      }
    });
  } else {
    // 태그가 없는 경우 기존 그대로 전체 렌더링
    renderAnswer(ansBox, text);
  }
  ...
  // 버튼 배치 및 폴백 처리
  if (hasTags && btnMore) {
    // 태그 매칭이 성공한 경우 더보기 버튼 삽입
    const refNode = wrap.querySelector(".orig") || foot;
    wrap.insertBefore(btnMore, refNode);
  } else {
    // 태그 매칭에 실패한 경우 기존의 스크롤 높이 기반 접기 폴백 활성화
    if (ansBox.scrollHeight > 180) {
      ansBox.classList.add("collapsible");

      const fade = el("div", "ans-fade");
      ansBox.appendChild(fade);

      const btn = el("button", "btn-more", "자세한 설명 더보기 ▾");
      btn.addEventListener("click", (ev) => {
        ev.stopPropagation();
        if (ansBox.classList.contains("expanded")) {
          ansBox.classList.remove("expanded");
          btn.textContent = "자세한 설명 더보기 ▾";
          wrap.scrollIntoView({ behavior: "smooth", block: "nearest" });
        } else {
          ansBox.classList.add("expanded");
          btn.textContent = "간략히 보기 ▴";
        }
      });
      
      const refNode = wrap.querySelector(".orig") || foot;
      wrap.insertBefore(btn, refNode);
    }
  }
