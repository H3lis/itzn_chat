...
def _run_retrieval_reranked(question: str, config: Config, backend: Backend) -> RetrievalResult:
    ...
    # --- TOC Expansion (Reference Expansion) ---
    import re
    pattern_range = r'(?:참고)?\s*\(?\s*(\d+)\s*(?:\\?~|-)\s*(\d+)\s*(?:쪽|페이지)\s*\)?\s*(?:참고)?'
    pattern_single = r'(?:참고)?\s*\(?\s*(\d+)\s*(?:쪽|페이지)\s*\)?\s*(?:참고)?'
    
    extra_pages = []
    ordered_keys = {(p["meta"].get("doc_slug"), int(p["meta"].get("page_number"))) for p in ordered}
    
    for p in list(ordered):
        doc_slug = p["meta"].get("doc_slug")
        for chunk in p.get("chunks", []):
            refs = set()
            for match in re.finditer(pattern_range, chunk.text):
                try:
                    s, e = int(match.group(1)), int(match.group(2))
                    if 0 < e - s < 10:  # 너무 큰 범위는 무시 (오탐 방지)
                        refs.update(range(s, e + 1))
                except ValueError:
                    pass
            for match in re.finditer(pattern_single, chunk.text):
                try:
            ...
            for page_num in refs:
                key = (doc_slug, page_num)
                if key not in ordered_keys:
                    ordered_keys.add(key)
                    extra_pages.append({
                        "meta": {"doc_slug": doc_slug, "page_number": page_num},
                        "score": p["score"] - 0.0001,  # 참조된 페이지는 원본보다 약간 낮게
                        "chunks": []
                    })
    
    if extra_pages:
        # 최대 4페이지만 추가 확장
        ordered.extend(extra_pages[:4])
    # -------------------------------------------
