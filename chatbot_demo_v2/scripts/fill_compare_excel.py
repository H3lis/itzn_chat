# -*- coding: utf-8 -*-
"""수정된 RAG 엔진으로 바탕화면의 무선망_개선RAG_비교.xlsx 엑셀 파일을 채운다."""
...
def main():
    excel_path = Path("c:/Users/hgcha/Desktop/무선망_개선RAG_비교.xlsx")
    ...
    print(f"총 데이터 행 수: {max_row - 1}개 (2행 ~ {max_row}행)")
    ...
    for r in range(2, max_row + 1):
        ...
        # 주의: 이전에 빈 칸이었던 요약/상세/근거를 새로 채울 수 있도록
        # 만약 이전에 Details가 없었다면 (이전 잘못된 실행 결과) 덮어쓰도록 설정
        ...
        # 제대로 모두 채워져 있는 경우에만 건너뜀 (중단 후 재개 시 유용)
        if rag_sum_val and rag_det_val and rag_src_val:
            skipped_count += 1
            continue

        question = str(q_val).strip()
        print(f"\n[{r-1}/{max_row-1}] 질문: {question[:40]}...")

        t0 = time.time()
        try:
            # 1. RAG 서브그래프를 통해 raw 초안 및 근거 획득
            res = ctx.rag_adapter.ask(question)
            base_answer = res.get("final_answer") or ""
            evidence_text = res.get("answer_context") or ""
            
            # 2. composer_rag 프롬프트를 통한 최종 답변 합성 (XML 태그 생성 유도)
            ...
            if s_match and d_match:
                ...
                # 태그 매칭에 실패한 경우 전체를 요약에 쓰고 상세를 비워둠 (또는 예외 처리)
    ...
    print("\n" + "="*40)
    print(f"작업 완료! 새로 입력: {success_count}건, 스킵: {skipped_count}건")
