...
def make_nodes(ctx: Any) -> dict[str, Callable[[ChatState], dict]]:
    ...
    # SpokenRobustnessEngine 지연/조건부 초기화
    spoken_engine = None
    if settings.robustness_tool_enabled:
        from ..robustness_tool import SpokenRobustnessEngine
        backend_wrapper = (lambda prompt: ctx.llm.chat(prompt)) if getattr(ctx, "llm", None) else None
        spoken_engine = SpokenRobustnessEngine(backend=backend_wrapper, config=settings)
    ...
    def normalize_input(state: ChatState) -> dict:
        ...
        if input_type == "action":
            ...
            if exact_entry is not None:
                ...
                out["user_input"] = raw
                out["normalized_question"] = norm_raw
                out["messages"] = [HumanMessage(content=raw)]
                out["trace"] = _trace(
                    {"trace": out["trace"]},
                    "normalize_input",
                    f"자유 입력 정규화(len={len(raw)}) | FAQ 정확 일치 우회",
                )
            elif spoken_engine is not None:
                # 구어체 및 오타 강건성 툴 적용
                _progress("robustness", "구어체 및 오타 자동 교정 중...")
                robust_res = spoken_engine.run(raw)
                final_query = robust_res["final_query"]

                out["user_input"] = final_query
                out["normalized_question"] = normalize_text(final_query)
                # 대화 메모리: 사용자 발화는 원본(raw) 보존하여 UI에 출력
                out["messages"] = [HumanMessage(content=raw)]

                detail_msg = f"자유 입력 정규화(len={len(raw)})"
                if robust_res["is_colloquial"] or robust_res["preprocessed_query"] != raw:
                    detail_msg += f" | 강건성 툴 적용: {raw} -> {final_query}"
                ...
                # LangSmith 트레이스용 메타데이터 기록
                _node_meta(
                    {
                        "original_query": raw,
                        "final_query": final_query,
                        "is_colloquial": robust_res["is_colloquial"],
                        "preprocessed_query": robust_res["preprocessed_query"],
                        "llm_query_rewritten": robust_res["llm_query_rewritten"],
                        "llm_query_rewrite_applied": robust_res["llm_query_rewrite_applied"],
                    },
                    tags=["robustness_tool"],
                )
            else:
